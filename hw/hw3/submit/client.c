...
char* range(char I_D[50][50], char x_position[50][50], char y_position[50][50], int destinationx, int destinationy, int reachable_number)
	{
		int i = 0,j;
...
char* range_recv(char I_D[50][50], char x_position[50][50], char y_position[50][50], int destinationx, int destinationy, int reachable_number, char datamessage_recv[50][50])
{
...
char** where(char *read3, int sock_num1)
{
    ...
    send(sock_num1, where, 100, 0);

    char where_recv[50][50];
	...
	where_recv_return = malloc(50 * sizeof (char *));
	int i;
	for(i = 0; i < 50; i++)
	{
		where_recv_return[i] = 	malloc(50 * sizeof (char));
	}
...
//MOVE function
char** move(char read[1024], int sock_num, char updateposition1[50][50])
{
	int n;
    ...
    char *position;
    position = malloc(50*sizeof(char));
    const char s[2] = " ";
  	strcpy(position, strtok(read, s));

	strcpy(position, strtok(NULL, s));
	for(int g = 0; g < sizeof(position); g++){
	if(position[g] == '\n'){
	      	position[g] = '\0';
	    }
	}
    ...
    memset(updateposition1[4],0,50);
    strcpy(updateposition1[4],move[2]);  //update current position

    //Send move to the server
    send(sock_num, updateposition1, 250, 0);

    char move_recv[50][50];
	...
	updateposition_return = malloc(50 * sizeof (char *));

	int i;

	for(i = 0; i < 50; i++)
	{
		updateposition_return[i] = 	malloc(50 * sizeof (char));
	}
...
//senddata function
void senddata(char *read1, int sock_num, char updateposition1[50][50])
{
	int n;
	send(sock_num, updateposition1, 250, 0);

    char senddata_recv[50][50];
	...
	for(int g = 0; g < sizeof(destination_ID); g++){
	    if(destination_ID[g] == '\n'){
	      	destination_ID[g] = '\0';
	    }
	}
	...
	// for(i = 0; i < 50; i++)
	// {
	// 	where_recv_senddata1[i] = malloc(50 * sizeof (char ));
	// }
	...
	char where_recv_senddata [50][50];
	memset(where_recv_senddata,0,sizeof(where_recv_senddata));

	strcpy(where_recv_senddata[0],"WHERE");
	strcpy(where_recv_senddata[1], destination_ID);
	// for (i = 0;i < 50; i++)
	// {
	...
	send(sock_num, where_recv_senddata, 100, 0);
    ...
    while(1)
    {
    	if((n = recv(sock_num, where_recv, sizeof(where_recv),0)) > 0)
    	{
    		if(strcmp(where_recv[0],"THERE") == 0)
    		{
    			break;
	...
	int destination_x = atoi(where_recv[2]);
	int destination_y = atoi(where_recv[3]);
	...
	char xposition[50][50];
	memset(xposition,0,sizeof(xposition));

	char yposition[50][50];
	...
	send(sock_num,senddata_message , 300, 0);
...
int main(int argc,char*argv[])
{
    ...
    //send UPDATEPOSITION
	send(sockfd, updateposition, 250, 0);  //5 * 50
    char sensor_reachable[50][50];
		...
		for(int w = 0; w < 2; w++)
		{
			...
			if((y = read(fd[w], recv_datamessage, sizeof(recv_datamessage)))>0)
			{
				...
				if(recv_datamessage[0][0] == 'M')
				{
					...
					memset(updateposition[3],0,50);
					memset(updateposition[4],0,50);
...

				{
					...
					//updateposition
					send(sockfd, updateposition, 250, 0);
					memset(sensor_reachable,0,sizeof(sensor_reachable));
					...
					if(strcmp(recv_datamessage[3], sensor_id) != 0)
					{
						...
						send(sockfd, datamessage_where, 100, 0);

						char datamessage_where_recv[50][50];
						...
						int destination_x = atoi(datamessage_where_recv[2]);
						int destination_y = atoi(datamessage_where_recv[3]);
						int u = 0;
						int reachablenumber;
						...
						memset(recv_datamessage[2],0,50);
						strcpy(recv_datamessage[2],next_id);
						memset(recv_datamessage[5+hoplist_length],0,50);
						...
						//renew hoplist
						memset(recv_datamessage[4],0,50);
						strcpy(recv_datamessage[4],hoplist_length_array);  //hoplist_length + 1

						int size = (hoplist_length + 5)*50;
						//send new datamessage to server
						send(sockfd, recv_datamessage, size, 0);
