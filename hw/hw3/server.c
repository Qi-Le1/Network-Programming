...
char*** readfile(char* filename, char*** base_info)
{
  ...
  for(int i = 0; i < 50; i++)
  {
    base_info[i] = malloc(50* sizeof(char*));
    for(int j = 0; j < 50; j++)
    {
      base_info[i][j] = malloc(50 * sizeof(char));
    }
  }
...
char** sensor_find_reachable(char*** sensorbuf, char*** basebuf, char sensorid[], int sensorcount, int basecount){
  ...
  for(int i = 0; i < 50; i++)
  {
    reachable[i] = malloc(50* sizeof(char));
    memset(reachable[i], 0, 50);
  }
...
char** filter_reachable(char** msg, char** reachable)
{
  ...
  for(int i = 0; i < 50; i++)
  {
    new_reachable[i] = malloc(50* sizeof(char));
  }
  int j = 2;
  int k = 2;
  if(strcmp(msg[4], "0") == 0){
...
char** new_msg(char origin_id[], char dest_id[], char*** base_buf, char*** sensor_buf, int base_count, int sensor_count)
{
  ...
  for(int i = 0; i < 50; i++)
  {
    datamsg[i] = malloc(50* sizeof(char));
  }
  int row_index = 2;
  int row;
  char* thisx = malloc(sizeof(char)*50);
  char* thisy = malloc(sizeof(char)*50);
...
char** deli_to_base(char** datamsg, char*** base_buf, char*** sensor_buf, int base_count, int sensor_count)
{
  ...
  for(int i = 0; i < 50; i++)
  {
    new_msg[i] = malloc(50* sizeof(char));
  }
  char* thisx = malloc(sizeof(char)*50);
  char* thisy = malloc(sizeof(char)*50);
...
char** find_best(char*** sensor_buf, char*** base_buf, int sensor_count, int base_count, char buffer[][50])
{
  char** the_id;
  int tmp1 = 2;
  int tmp2 = 2;
  bool found1 = false;
  bool found2 = false;
  the_id = malloc(50 * sizeof(char*));
  for(int i = 0; i < 50; i++)
  {
    the_id[i] = malloc(50* sizeof(char));
  }
  char** reachable1;
  reachable1 = malloc(50 * sizeof(char*));
  for(int i = 0; i < 50; i++)
  {
    reachable1[i] = malloc(50* sizeof(char));
  }
  char** reachable2;
  reachable2 = malloc(50 * sizeof(char*));
  for(int i = 0; i < 50; i++)
  {
    reachable2[i] = malloc(50* sizeof(char));
  }
  ...
  if((row = sensor_isfound(buffer[2], sensor_buf, sensor_count)) != -1){
    reachable1 = sensor_find_reachable(sensor_buf, base_buf, buffer[2], sensor_count, base_count);
  }
  if((row = base_isfound(buffer[2], base_buf, base_count)) != -1){
    reachable1 = base_find_reachable(sensor_buf, base_buf, buffer[2], sensor_count, base_count);
  }
  if((row = sensor_isfound(buffer[3], sensor_buf, sensor_count)) != -1){
    reachable2 = sensor_find_reachable(sensor_buf, base_buf, buffer[3], sensor_count, base_count);
  }
  if((row = base_isfound(buffer[3], base_buf, base_count)) != -1){
    reachable2 = base_find_reachable(sensor_buf, base_buf, buffer[3], sensor_count, base_count);
  }
  while(tmp1 < 2 + 3*atoi(reachable1[1])){
    if(strcmp(reachable1[tmp1], buffer[4]) == 0){
      found1 = true;
    }
    tmp1 += 3;
  }
  while(tmp2 < 2 + 3*atoi(reachable2[1])){
    if(strcmp(reachable1[tmp2], buffer[4]) == 0){
      found2 = true;
    }
    tmp2 += 3;
  }
  ...
  strcpy(the_id[0], "EQUAL");
  if(found1 == false && found2 == true){
    strcpy(the_id[1], buffer[3]);
    return the_id;
  }else{
    strcpy(the_id[1], buffer[2]);
    return the_id;
  }
...
int main(int argc, char* argv[])
{
  ...
  for(int i = 0; i < 50; i++)
  {
    ...
    for(int j = 0; j < 50; j++)
    {
      base_buf[i][j] = malloc(50 * sizeof(char));
    }
  }
  ...
  for(int i = 0; i < 50; i++)
  {
    ...
    for(int j = 0; j < 50; j++)
    {
      sensor_buf[i][j] = malloc(50 * sizeof(char));
    }
  }
  ...
  char stdin_buf[50][50];
  memset(stdin_buf, 0, sizeof(stdin_buf));
  // char msg[10][50];
  // memset(msg, 0, sizeof(msg));
  char reachable[50][50];
  ...
  int serverport = atoi(argv[1]);
	serverfd = create_addr(serverport);
        ...
        for (int i = 0; i < 50; i++)
        {
          ...
          if (FD_ISSET(clifd, &rset))
          {
            ...
            if((n = read(client[i].fd, buffer, sizeof(buffer))) > 0)
            {
              ...
              //deal with equal from client
              if(strcmp(buffer[0], "EQUAL") == 0)
              {
                char** equal;
                char equaltosend[50][50];
                equal = find_best(sensor_buf, base_buf, sensor_count, base_count, buffer);
                for(int w = 0; w < 50; w++){
                  strcpy(equaltosend[w], equal[w]);
                }
                send(client[i].fd, equaltosend, 200, 0);
                for(int j = 0; j < 50; j++){
                  free(equal[j]);
                }
                free(equal);
                memset(equaltosend, 0, sizeof(equaltosend));
              ...
              if(strcmp(buffer[0], "UPDATEPOSITION") == 0)
              {
                ...
                send(client[i].fd, reachable, 600, 0);
                memset(reachable, 0, sizeof(reachable));
                memset(buffer, 0, sizeof(buffer));
                for(int j = 0; j < 50; j++){
                  free(reachable2[j]);
              ...
              if(strcmp(buffer[0], "DATAMESSAGE") == 0)
              {
                if((row = base_isfound(buffer[2], base_buf, base_count)) != -1)
                {
                  ...
                  for(int k = 0; k < 50; k++)
                  {
                    msg_to_deli[k] = malloc(50* sizeof(char));
                  }
                  for(int j = 0; j < 50; j++){
