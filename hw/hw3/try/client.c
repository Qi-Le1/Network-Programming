...
char* range(char I_D[50][40], char x_position[50][40], char y_position[50][40], int destinationx, int destinationy, int reachable_number)
	{
		int i = 0,j;
...
char* range_recv(char I_D[50][40], char x_position[50][40], char y_position[50][40], int destinationx, int destinationy, int reachable_number, char datamessage_recv[50][40])
{
...
char** where(char *read3, int sock_num1)
{
    ...
    send(sock_num1, where, 80, 0);

    char where_recv[50][40];
	...
	for(i = 0; i < 50; i++)
	{
		where_recv_return[i] = 	malloc(40 * sizeof (char));
	}
...
//MOVE function
char** move(char read[1024], int sock_num, char updateposition1[50][40])
{
	int n;
    ...
    memset(updateposition1[4],0,40);
    strcpy(updateposition1[4],move[2]);  //update current position

    //Send move to the server
    send(sock_num, updateposition1, 200, 0);

    char move_recv[50][40];
	...
	for(i = 0; i < 50; i++)
	{
		updateposition_return[i] = 	malloc(40 * sizeof (char));
	}
...
//senddata function
void senddata(char *read1, int sock_num, char updateposition1[50][40])
{
	int n;
	send(sock_num, updateposition1,200, 0);

    char senddata_recv[50][40];
	...
	char where_recv_senddata [50][40];
	...
	char xposition[50][40];
	memset(xposition,0,sizeof(xposition));

	char yposition[50][40];
	...
	send(sock_num,senddata_message, 240, 0);
...
int main(int argc,char*argv[])
{
    ...
    //send UPDATEPOSITION
	send(sockfd, updateposition, 200, 0);
    char sensor_reachable[50][40];
		...
		for(int w = 0; w < 2; w++)
		{
			...
			if((y = read(fd[w], recv_datamessage, sizeof(recv_datamessage)))>0)
			{
				...
				if(recv_datamessage[0][0] == 'M')
				{
					...
					memset(updateposition[3],0,40);
					memset(updateposition[4],0,40);
...

				{
					...
					if(strcmp(recv_datamessage[3], sensor_id) != 0)
					{
						...
						send(sockfd, datamessage_where, 80, 0);

						char datamessage_where_recv[50][40];
						...
						memset(recv_datamessage[2],0,40);
						strcpy(recv_datamessage[2],next_id);
						memset(recv_datamessage[5+hoplist_length],0,40);
